...
export const DEMO_LABEL = 'DEMO · ILLUSTRATIVE FORECAST DATA · NOT LIVE';
...
export class DemoDataRepository {
  ...
  async getPublicState() {
    ...
    const questionsMatchSeed = questions.rows.every((row, index) => row.id === demoQuestions[index]?.id && row.display_id === demoQuestions[index]?.displayId);
    const viewsMatchSeed = views.rows.every((row, index) => row.label === demoViews[index]?.label);
    const evidenceMatchesSeed = evidence.rows.every((row, index) => row.id === demoEvidence[index]?.id);
    if (!meta || meta.migration_version !== DEMO_MIGRATION_VERSION || meta.seed_version !== DEMO_SEED_VERSION
      ...
      || !questionsMatchSeed || !viewsMatchSeed || !evidenceMatchesSeed) {
