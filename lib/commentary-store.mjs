...
export class CommentaryStore {
  ...
  submitComment({ memberSub, questionId, body, consent }) {
    ...
    if (normalized.length < 20 || Buffer.byteLength(normalized, 'utf8') > 1500 || /[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/.test(normalized)) {
      throw appError('Commentary must be valid plain text between 20 and 1500 UTF-8 bytes', 400);
    }
    const nowMs = Date.parse(this.now());
