...
Rebuilt: subject-first cinematic type, a code-built physical-to-cloud supply-chain plate, historical route, connected signal waveform, dark measurable-question chapter, single season slate, one host portrait, and compact method.
...
Episode 01 has a public editorial question, threshold, deadline, and evidence class. Reader YES/NO choices are local browser state only. Sharing derives from canonical metadata and follows an open season question. No public audience aggregate, playback, commentary, or forecast ledger is claimed.
...
- **Tick 1 — implementation and automated gates:** complete when the local commit named in the handoff is created.
