...
import pg from 'pg';
import { DemoDataRepository } from './lib/demo-data-repository.mjs';
...
async function handleApi(req, res, url) {
  ...
  try {
    if (url.pathname === '/api/demo-state') {
      ...
      if (!demoMode || !demoRepository) return json(res, 503, { demo: true, label: 'DEMO — Illustrative sample data', error: 'Demo data unavailable' });
      try {
        ...
        return json(res, 503, { demo: true, label: 'DEMO — Illustrative sample data', error: 'Demo data unavailable' });
      }
    }
...
const server = createServer(async (req, res) => {
  ...
  if (url.pathname === '/readyz') {
    const ready = !demoMode || (demoRepository && await demoRepository.readiness());
